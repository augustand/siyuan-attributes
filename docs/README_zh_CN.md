...
注意：当前版本聚焦文档属性。数据库字段功能已暂时移除。

### 即将推出的功能...

- 块菜单对话框已可用：从块标菜单或内容右键菜单打开**属性面板**，编辑该块的 `custom-*` 属性
- [内联显示块级属性](https://github.com/InEase/SiYuan-Attributes-Panel/issues/7)（跟踪中）
...
1. 在文档下面显示一个属性面板，展示**文档级**自定义属性；自定义属性支持新增、修改、删除；新增时只需输入名称，保存前会自动补 `custom-` 前缀
2. **属性面板设置**（插件设置）：配置全局默认规则（显示名、显示/隐藏、可编辑性、排序、通配/正则）
3. **字段设置**（面板内）：仅覆盖**当前文档**的字段显示；未覆盖的字段仍使用全局默认
4. 支持暗黑模式
5. 从块标菜单（及内容右键菜单）打开对话框，编辑该块的 `custom-*` 属性；使用全局规则；不支持块级字段覆盖

内部会使用保留属性 `custom-mux-attrs-doc-fields` 存储文档级覆盖，该键不会出现在属性面板中。
